                        ...
                        <div class="col-12">
                            ...
                            <div class="offcanvas offcanvas-start" tabindex="-1" id="offcanvasCategorias" aria-labelledby="offcanvasCategoriasLabel">
                                <div class="offcanvas-header">
                                    <h5 class="offcanvas-title" id="offcanvasCategoriasLabel">{{ __('Categorias') }}</h5>
                                    <button type="button" class="btn-close" data-bs-dismiss="offcanvas" aria-label="Close"></button>
                                </div>
                                <div class="offcanvas-body">
                                    <p>{{ __('Selecione uma categoria') }}:</p>
                                    <ul class="list-unstyled mb-0 p-0">
                                        @foreach ($categories->get() as $category)
                                            <li>
                                                <a href="{{ route_lang('blog', ['slug' => $category->slug]) }}" class="@if (route_lang('blog', ['slug' => $category->slug]) === Request::fullUrl()) active @endif">{{ $category->name }}</a>
                                            </li>
                                        @endforeach
                                        <li>
                                            <a href="{{ route_lang('blog') }}" class="fw-bold">
                                                {{ __('Ver todos') }}
                                            </a>
                                        </li>
                                    </ul>
